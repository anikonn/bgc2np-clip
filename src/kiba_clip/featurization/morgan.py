...
class MorganFingerprintFeaturizer:
    """Compute Morgan (ECFP) fingerprints for SMILES."""

    def __init__(self, cfg: MorganConfig) -> None:
        self.cfg = cfg
        # New RDKit API (no deprecation warning)
    ...
    def encode(self, smiles: str) -> torch.Tensor:
        """Return fingerprint as float tensor [n_bits]."""
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            raise ValueError(f"Invalid SMILES: {smiles}")

        fp = self._fpgen.GetFingerprint(mol)  # ExplicitBitVect
        arr = np.zeros((self.cfg.n_bits,), dtype=np.float32)
        ConvertToNumpyArray(fp, arr)
        return torch.from_numpy(arr)  # already float32